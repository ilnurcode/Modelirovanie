...
func TestLauncherKeepsInstallerManagementAvailable(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(appDir, appName), []byte("app"), 0o755); err != nil { t.Fatal(err) }; if err := os.WriteFile(filepath.Join(installerDir, installerName), []byte("installer"), 0o755); err != nil { t.Fatal(err) }; if err := os.WriteFile(node, []byte("node"), 0o755); err != nil { t.Fatal(err) }; if err := os.WriteFile(cli, []byte("pi"), 0o644); err != nil { t.Fatal(err) }
